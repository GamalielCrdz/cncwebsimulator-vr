const { GCode, GCodeModel, GWord } = require('./gcodeModel');
...
GCodeParser.prototype.parseWord = function (word) {
	...
	var letter = word.word[0].toUpperCase(),
		value;

	if ((letter < 'A') || (letter > 'Z')) {
		throw new Error('Unexpected command letter: ' + letter + ' from word: ' + word.word);
	}

	value = word.word.slice(1);
	// value = parseFloat(word.slice(1));
	// if (isNaN(value)) {
	//   throw new Error('Bad number format: ' + value);
	// }

	return new GWord(letter, value, word.word, word.lineNumber);
};
...
GCodeParser.prototype.parseLine = function (line) {	
	...
	pLine.comments = self.parseComments(line.line);
	pLine.comments.forEach(function (comment) {
		// console.log("Removing comment: " +  comment);
		line.line = line.line.replace(comment, '');
	});

	words = line.line.trim().split(' ');	
	...
	for (; i < l; i++) {
		...
		if (!words[i].word || words[i].word.length <= 0) {
